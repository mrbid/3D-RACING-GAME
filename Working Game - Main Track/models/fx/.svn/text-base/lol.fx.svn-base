...
/*** Generated through Lumonix shaderFX  by: h8101260 in 3dsmax at: 09/03/2011 00:52:12  ***/ 
...
sampler2D TextureMap_218Sampler = sampler_state
{
	Texture = <TextureMap_218>;
	MinFilter = LINEAR;
	MagFilter = LINEAR;
...
{ 
	...
	if ( lightType == 2 ) lightVec = mul(objTangentXf, (mul(lightVector, worldI).xyz)); 
	return lightVec; 
} 
...
{ 
	...
	float4 TextureMap_218 = tex2D(TextureMap_218Sampler, In.texCoord.xy);
	float3 input2 = TextureMap_218.rgb; 
...
{  
	pass ambient  
    {		 
		VertexShader = compile vs_3_0 av(light1Dir,  light1Type, light1Dir); 
		ZEnable = true; 
		ZWriteEnable = true; 
		CullMode = cw; 
		ShadeMode = Gouraud;
	...
	pass light1  
    {		 
		VertexShader = compile vs_3_0 v(light1Dir,  light1Type, light1Dir); 
		ZEnable = true; 
		CullMode = cw; 
		ShadeMode = Gouraud;
		ZWriteEnable = true; 
